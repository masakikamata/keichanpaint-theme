<?php
/**
 * Plugin Name: けいちゃんペイント 助成金管理（神奈川）
 * Plugin URI:  https://keichanpaint-main.group
 * Description: 神奈川県 市町村別 助成金情報を管理画面から編集し、フロントの助成金マップに反映します。
 * Version:     1.0.0
 * Author:      けいちゃんペイント
 *
...
const KEICHAN_GRANT_KANAGAWA_GITHUB_DEFAULT_URL = 'https://raw.githubusercontent.com/masakikamata/keichanpaint-theme/main/kanagawa_josei_copy.html';

/* ============================================================
   1. 市町村マスター（神奈川県 全33市町村）
   ============================================================ */
function keichan_grant_kanagawa_cities(): array {
    return [
        // ── 政令指定都市（3） ──
        '横浜市', '川崎市', '相模原市',
        // ── 一般市（16） ──
...
function keichan_grant_kanagawa_github_url(): string {
    $url = get_option( 'keichan_grant_kanagawa_github_url', '' );
    return $url ?: KEICHAN_GRANT_KANAGAWA_GITHUB_DEFAULT_URL;
}

function keichan_grant_kanagawa_strip_tags_clean( string $s ): string {
    $s = preg_replace( '/<[^>]+>/u', ' ', $s );
 ...
 */
function keichan_grant_kanagawa_parse_html( string $html ): array {
    $result = [];
    ...
    foreach ( $rows as $row ) {
        ...
        $result[ $city ] = [
            'grant_name' => mb_substr( keichan_grant_kanagawa_strip_tags_clean( $cells[1] ), 0, 200 ),
            'period'     => mb_substr( keichan_grant_kanagawa_strip_tags_clean( $cells[2] ), 0, 100 ),
            'slots'      => mb_substr( keichan_grant_kanagawa_strip_tags_clean( $cells[3] ), 0, 100 ),
            'url'        => $url,
            'pdf_url'    => $pdf_url,
        ];
    }
 ...
 */
function keichan_grant_kanagawa_sync_from_github( bool $force = false ): array {
    $url = keichan_grant_kanagawa_github_url();
    if ( empty( $url ) ) {
    ...
    update_option( 'keichan_grant_kanagawa_github_checked', current_time( 'Y-m-d H:i:s' ), false );

    $last_hash = get_option( 'keichan_grant_kanagawa_github_hash', '' );
    if ( ! $force && $hash === $last_hash ) {
        return [ 'status' => 'nochange', 'message' => '変更なし（前回と同じ内容）' ];
    }

    $parsed = keichan_grant_kanagawa_parse_html( $body );
    if ( empty( $parsed ) ) {
        return [ 'status' => 'error', 'message' => 'HTMLからデータを抽出できませんでした' ];
    }

    $cities         = keichan_grant_kanagawa_cities();
    ...
    update_option( 'keichan_grant_kanagawa',          $data );
    update_option( 'keichan_grant_kanagawa_github_hash',    $hash,                          false );
    update_option( 'keichan_grant_kanagawa_github_synced',  current_time( 'Y-m-d H:i:s' ), false );
    update_option( 'keichan_grant_kanagawa_updated',  current_time( 'Y-m-d' ) );
...
add_filter( 'cron_schedules', function ( $schedules ) {
    if ( ! isset( $schedules['keichan_kanagawa_monthly'] ) ) {
        $schedules['keichan_kanagawa_monthly'] = [
...
add_action( 'init', function () {
    if ( ! wp_next_scheduled( 'keichan_grant_kanagawa_github_cron' ) ) {
        wp_schedule_event( time() + HOUR_IN_SECONDS, 'keichan_kanagawa_monthly', 'keichan_grant_kanagawa_github_cron' );
    }
} );

add_action( 'keichan_grant_kanagawa_github_cron', function () {
    keichan_grant_kanagawa_sync_from_github( false );
} );
...
add_action( 'admin_menu', function () {
    add_management_page(
        ...
        '🗾 神奈川助成金管理',
        'manage_options',
        'keichan-grant-kanagawa',
        'keichan_grant_kanagawa_admin_page'
    );
} );
...
/* ============================================================
   3. 管理画面
   ============================================================ */
function keichan_grant_kanagawa_admin_page(): void {
    ...
    /* ---- GitHub取り込みハンドラ ---------------------------- */
    if (
        isset( $_POST['kgk_github_nonce'] ) &&
        wp_verify_nonce( sanitize_text_field( wp_unslash( $_POST['kgk_github_nonce'] ) ), 'keichan_grant_kanagawa_github' )
    ) {
        // URL設定を更新
        $new_url = esc_url_raw( wp_unslash( $_POST['kgk_github_url'] ?? '' ) );
        update_option( 'keichan_grant_kanagawa_github_url', $new_url, false );

        $force  = isset( $_POST['kgk_github_force'] );
        $result = keichan_grant_kanagawa_sync_from_github( $force );
    ...
    /* ---- 保存処理 ------------------------------------------ */
    if (
        isset( $_POST['keichan_grant_kanagawa_nonce'] ) &&
        wp_verify_nonce( sanitize_text_field( wp_unslash( $_POST['keichan_grant_kanagawa_nonce'] ) ), 'keichan_grant_kanagawa_save' )
    ) {
        $cities  = keichan_grant_kanagawa_cities();
        ...
        update_option( 'keichan_grant_kanagawa',         $payload );
        update_option( 'keichan_grant_kanagawa_updated', current_time( 'Y-m-d' ) );
    ...
    /* GitHub関連情報 */
    $github_url     = keichan_grant_kanagawa_github_url();
    $github_synced  = get_option( 'keichan_grant_kanagawa_github_synced',  '未取得' );
    $github_checked = get_option( 'keichan_grant_kanagawa_github_checked', '未取得' );
    $next_cron      = wp_next_scheduled( 'keichan_grant_kanagawa_github_cron' );
    ...
    /* ---- フロントページURL ---------------------------------- */
    $front_page = get_page_by_path( 'grant-kanagawa' );
    $front_url  = $front_page ? get_permalink( $front_page->ID ) : '';

    /* ---- フォーム送信先 ------------------------------------ */
    $action_url = esc_url( admin_url( 'tools.php?page=keichan-grant-kanagawa' ) );
    ...
    .kg-section.town{background:#7b3f8a;}
    ...
    <div id="kg-wrap" class="wrap">
        ...
        <form method="post" action="<?php echo $action_url; ?>" class="kg-github-box" id="kg-github-form">
            <?php wp_nonce_field( 'keichan_grant_kanagawa_github', 'kgk_github_nonce' ); ?>
            ...
            <div class="gh-row">
                <label for="kgk_github_url">Raw URL:</label>
                <input type="url" id="kgk_github_url" name="kgk_github_url"
                       value="<?php echo esc_attr( $github_url ); ?>"
                       placeholder="https://raw.githubusercontent.com/.../*.html">
                <button type="submit" name="kgk_github_sync" class="button button-primary">📥 今すぐ取り込む</button>
                <button type="submit" name="kgk_github_force" class="button"
        ...
        <form method="post" action="<?php echo $action_url; ?>" id="kg-form">
            <?php wp_nonce_field( 'keichan_grant_kanagawa_save', 'keichan_grant_kanagawa_nonce' ); ?>

            <?php
            $sections = [
                [ 'label' => '政令指定都市',   'class' => '',     'cities' => array_slice( $cities, 0, 3 ) ],
                [ 'label' => '一般市',         'class' => 'tama', 'cities' => array_slice( $cities, 3, 16 ) ],
                [ 'label' => '町村',           'class' => 'town', 'cities' => array_slice( $cities, 19 )    ],
            ];
